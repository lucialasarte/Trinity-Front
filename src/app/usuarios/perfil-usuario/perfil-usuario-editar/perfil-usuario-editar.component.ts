...
export class EditarUsuarioComponent implements OnInit {
  ...
  constructor(
    ...
    private modalRef: NzModalRef<EditarUsuarioComponent>, // Inyecta NzModalRef
  ...
  private _loadUsuario(id: number): void {
    this.usuariosService.getUsuarioPorId(id).subscribe({
      next: (usuario: Usuario) => {
        ...
        while (this.tarjetas.length !== 0) {
          this.tarjetas.removeAt(0);
        }


        if (usuario.roles[0].id == 3) {
          const tarjetaFormGroup = this._createTarjetaFormGroup();
          tarjetaFormGroup.patchValue(usuario.tarjetas[0]); // Llenar con los datos de la tarjeta
          this.tarjetas.push(tarjetaFormGroup);
        
            this.imagenes = usuario.id_imagenes?.map((img: any) => ({
                id: img,
                url: `${this.apiUrl}/imagenDoc/${img.id}`,
            })) || [];
        }
